...
public class BladeScriptTwoPlayerUp : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            if(gameObject.name.Equals("Blade Up"))
            {
                ...
                if(touchPosition.y > 0f)
                {
                    ...
                    positionVector.y = Mathf.Clamp(touchPosition.y, 0f, 4f);
    ...
    {
        if (collision.collider.transform.position.y < 0f)
        {
            score++;
            scoreText.text = score.ToString();
        }

        else if (collision.collider.transform.position.y > 0f)
        {
            scoreUp++;
